...
# Installation
```bash
go get -u github.com/quangdangfit/go-queryset/cmd/goqueryset
```

# Usage
## Define models
Imagine you have model `User` in your [`models.go`](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm1/gorm1.go#L16) file:
...
Now transform it by [adding comments for query set generation](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L15):
```go
...
And you will get file [`autogenerated_models.go`](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go) in the same directory (and package) as `models.go`.
...
See full autogenerated file [here](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go).
...
```
Under the hood `Create` method [just calls `db.Create(&u)`](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L38).
...
Goqueryset generates DB names for struct fields into [`UserDBSchema`](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L414) variable.
In this example we used [`UserDBSchema.Rating`](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L419).
...
## Why not any ORM?
I didn't see any ORM that properly handles code duplication. GORM is the best with `Scopes` support, but even it's far from ideal. E.g. we have GORM and [next typical code](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm1/gorm1.go#L16):
```go
type User struct {
	gorm.Model
	Rating      int
...
How to solve it? First idea is to [make reusable functions](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm2/gorm2.go#L27):
```go
...
We can use GORM [Scopes](http://jinzhu.me/gorm/crud.html#scopes) to improve [how it looks](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm3/gorm3.go#L64):
```go
...
Now compare it with [go-queryset solution](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L30):
```go
...
* QuerySet pattern allows to reuse queries by defining [custom methods](https://github.com/quangdangfit/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L30) on it.
